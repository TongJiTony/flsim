...
def parse_log(log_file, max_round, loss_flag):
    """ 解析日志文件，提取轮数和准确率 """
    rounds = []
    accuracies = []
    ...
    if loss_flag:
        accuracy_pattern = re.compile(r"Loss: (\d+\.\d+)")
...
def process_logs(log_files, output_file, max_round, loss_flag):
    ...
    for log_file in log_files:
        ...
        rounds, accuracies = parse_log(log_file, max_round, loss_flag)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process log files to compute average accuracy or loss.")
    ...
    process_logs(args.files, args.output, args.rounds, args.loss)